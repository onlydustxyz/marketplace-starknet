...
func migrate{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}():
    ...
    stream.foreach_struct(
        migrate_single_contribution, contributions_len, contribs, DeprecatedContribution.SIZE
    )

    return ()
end

func migrate_single_contribution{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    index : felt, el : DeprecatedContribution*
):
...
func all_deprecated_contributions{
    ...
    let (contributions_len) = fetch_contribution_loop(contribution_count, contributions)
...
func fetch_contribution_loop{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    let (local contributions_len) = fetch_contribution_loop(contribution_index - 1, contributions)
